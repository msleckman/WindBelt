...
projects_words_NU <- project_pdfs_full_text %>%
  tidytext::unnest_tokens(output = word, input = text, token = 
                          "words", to_lower = T) %>%   # important to put all words to lower because it AFINN dictionnary has words      
...
length(unique(projects_score_bind_NU$document)) # 346
length(unique(projects_words_count_NU$document)) # 349 -->  three projects get removed when filtering all NA scores. This indicates thatthree projects have no words, or only words with no sentiment 

# What projects
b <- unique(projects_score_bind_NU$document)
a <- unique(projects_words_count_NU$document)
setdiff(a,b)

# the project that get taken out as a result of word parsing and filtering of NA sentiment scores 
# score DF nonfiltered
projects_score_bind_NU <-projects_words_count_NU %>% 
  left_join(get_sentiments("afinn"), by = "word")

  # 1. "Allendorf_NorthernAlternativeEnergy_1(2).PDF"
View(projects_score_bind_NU %>% 
  filter(document == "Allendorf_NorthernAlternativeEnergy_1(2).PDF")
    )

  # 2. "GreatPathfinderWind_ApexCleanEnergy_1(4).PDF"
View(projects_score_bind_NU %>% 
       filter(document == "GreatPathfinderWind_ApexCleanEnergy_1(4).PDF")
     )
## yea...pdf has nothing in it really
   
  # 3. "RidgeEnergyProject_RidgeEnergy_1.PDF"  
View(projects_score_bind_NU %>% 
       filter(document == "RidgeEnergyProject_RidgeEnergy_1.PDF")
     )
...
total_sentiment_NU_df <- total_sentiment_NU_df %>% 
  mutate(ProjectName = sub("\\_.*$", "", document))
    # generate merge with extracted_projectName
    #verify: may be spaces in Ventyx project name

View(ventyx_df)
View(total_sentiment_NU_df)

#Merge with original data (ventyx_df)
ventyx_df_NU_Sen <- merge(ventyx_df, total_sentiment_NU_df, by = "ProjectName", all = TRUE)
