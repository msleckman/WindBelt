...
```{r regression_processing, child="Regression_"}
...
# View(google_df[grep("Osborn*", google_df$ProjectName),])
...
dwplot(regression1[!grepl("^State*|View_Score|PopDensity_mi|Household_MedianIncome|Capacity", regression1$term),]%>% 
   relabel_predictors(renamed_variables), 
   dot_args = list(colour = "firebrick"),
   whisker_args = list(colour = "firebrick", lwd = 0.5)
   )+
...
dwplot(regression1[!grepl("^State*|H_L_WLow|Google_Sentiment|NU_Sentiment", regression1$term),] %>% 
   relabel_predictors(renamed_variables), 
   dot_args = list(colour = "firebrick"),
   whisker_args = list(colour = "firebrick", lwd = 0.5)
   )+
...
google_sentiments <- ventyx_df_NU_google_Sen$Google_Sentiment
mean(google_sentiments) #0.1610461
std(google_sentiments) #0.01574453
sd(google_sentiments)
plot(ventyx_df_NU_google_Sen$TimelineDays, ventyx_df_NU_google_Sen$Google_Sentiment)

#Residuals
hist(resid(reg_GSen_VS_PopDen_MedianIncome_Cap_HL_State_Memberships_Inter), main = "", xlab = "Residuals") 
plot(resid(reg_GSen_VS_PopDen_MedianIncome_Cap_HL_State_Memberships_Inter), ylab = "Residuals")
plot(reg_GSen_VS_PopDen_MedianIncome_Cap_HL_State_Memberships_Inter)

#timeline days details
plot(ventyx_df_NU_google_Sen$TimelineDays)
mean(ventyx_df_NU_google_Sen$TimelineDays)
std(ventyx_df_NU_google_Sen$TimelineDays)
sd(ventyx_df_NU_google_Sen$TimelineDays)

#variance inflation factor (for multicollinearity)
vif(reg_GSen_VS_PopDen_MedianIncome_Cap_HL_State_Memberships_Inter) #none of them higher than 4, so only moderate collinearity that doesn't need to be addressed

#correlation test
cor.test(ventyx_df_NU_google_Sen$TimelineDays, ventyx_df_NU_google_Sen$Google_Sentiment, method = "pearson") #low correlation between timeline and google sentiment (0.0777)
```

```{r r14_interaction}

interaction_term_plot<- plot_model(reg_GSen_VS_PopDen_MedianIncome_Cap_HL_State_Memberships_Inter,
             type = "int", mdrt.values = "meansd",
             axis.title = "", title = "", 
             legend.title = "",
             line.size = 2, 
  ...
  #theme_classic() + 
    theme(
        axis.title.y.right = element_text(size=25, vjust = -2, colour = "white"),
        axis.title.y.left = element_text(size=25, vjust = -2, colour = "white"),
        axis.text.y = element_text(size = 20, hjust=1, colour = "white"),
        axis.text.x = element_text(size = 20, hjust=0.5, vjust = -1, colour = "white"),
        axis.title.x.bottom = element_text(size=24, vjust = -3.5,  margin = margin(t = -2, r = 20, b = 35, l = 0), colour = "white"),
        axis.ticks = element_line(size = 2, colour = "white"),
        # axis.ticks.margin =  unit(c(1,-1),'cm'),
        axis.ticks.length=unit(0.2,"cm"),
        axis.line = element_line(colour = 'white', size = 2),
