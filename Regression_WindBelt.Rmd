...
# Pulling all pdfs directly for Gdrive 
...
#Directory path if downloaded pdfs from google drive using the in the chunk above
directory1 <- 'H:/Desktop/NU_PDFS_R'

# listing all pdfs in the NU_PDFs_R folder
pdfs <- paste(directory1, "/", list.files(directory1, pattern = "*.pdf", ignore.case = T), sep = "")
  # should be 349 pdfs_from NU

#pdfs names
pdfs_names <- list.files(directory1, pattern = "*.pdf", ignore.case = T)
#pdfs_names

#extract all text from all pdfs in NU_PDFs_R
pdfs_text <- purrr::map(pdfs, pdftools::pdf_text)
    # fyi takes a minute...
    # expect 9 'PDF error' and ignore. text normally processed still

```

#### Dataframe 1 with just pdfs and full text of the pdf
```{r dataframe1_creation}

##### NU

#each row is a pdf doc name (document) with the full pdf text
projects_pdftext_NU <- data_frame(document = pdfs_names, text = pdfs_text)
...
# dataset with each in one row
project_pdfpages_NU <- projects_pdftext_NU %>% 
  unnest() # splits pdf text by page and removes list format ( c("")) since each element of the list is now its own row.
View(project_pdfpages_google)


# dataset with each page in one row
project_pdfpages_google <- projects_pdftext_google %>% 
  unnest() # splits pdf text by page and removes list format ( c("")) since each element of the list is now its own row.
View(project_pdfpages_google)
...
projects_pdfwords_NU <- projects_pdftext_NU %>% 
  unnest() %>% 
...
#Dataset with each work in or row associated with its pdf source 
# seems like for google,  you don't need these filters - i commented out

  # projects_pdfwords_google <- projects_pdftext_google %>% 
  #   unnest() %>% 
  #   tidytext::unnest_tokens(output = word, input = text, token = 
  #                           "words", to_lower = T
  #                 # strip_numeric = TRUE
...
### NU 
projects_pdfwords_count_NU <- projects_pdfwords_NU %>%
...
### Google 
projects_pdfwords_count_google <- projects_pdfwords_google %>%
  group_by(document, word) %>% 
  summarise(count = n())
# View(projects_pdfwords_count)
# counts the number of time a specific words is found in the page pdf.
...
#### NU ####
projects_score_bind_NU <-projects_pdfwords_count_NU %>% 
...
#### google ####
projects_score_bind <-projects_pdfwords_count_google %>% 
  left_join(get_sentiments("afinn"), by = "word")
...
##### NU ######
total_sentiment1_NU <- projects_score_bind_NU %>% 
  filter(score !="NA") %>% 
  group_by(document) %>% 
...
#### Google #####
total_sentiment1_google <- projects_score_bind_google %>% 
  filter(score !="NA") %>% 
  group_by(document) %>% 
  summarise(totals = weighted.mean(score, w = count),
            standard_dev = sd(score), 
