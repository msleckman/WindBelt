...
```{r, child = 'Regression_WindBelt_processing_markdown.Rmd'}

```

```{r, child = 'Regression_Windbelt_runs_plots.Rmd'}
...
projects_words_nu_totalwords <- projects_words_NU %>% 
  group_by(document) %>% 
  summarise(word_count_per_project = n())

  View(projects_words_nu_totalwords)

projects_words_google_totalwords <- projects_words_google %>% 
  group_by(document) %>% 
  summarise(word_count_per_project = n())

View(projects_words_google_totalwords)
...
projects_score_bind_NU_2 <- projects_words_NU %>% 
  left_join(get_sentiments("afinn"), by = "word") %>% 
  # filter(score != "NA") %>% 
  filter(!is.na(score)) 
...
View(head(total_sentiment_google))
View(head(total_sentiment_with_stats_google))  
...
#Remove variance and sd before merge
# ventyx_df_NU_Sen <- ventyx_df_NU_Sen %>% 
#   select(-variance, -standard_dev)
...
# read_csv("G:/Data/ventyx_df_NU_google_Sen.csv")
...
dwplot(regression3[!grepl("^State*|View_Score|PopDensity_mi|Household_MedianIncome|Capacity|NU_Sentiment|NU_Sentiment:H_L_WLow", regression3$term),]%>% 
 relabel_predictors(renamed_variables), 
   dot_args = list(colour = "#93c47e", lwd = 1.5),
   whisker_args = list(colour = "#93c47e", lwd = 0.8)
   )+
  labs(x = "Timeline (days)", y = "")+
...
```{r R14}
####14. Timeline on left-hand side, using Google sentiment. Interaction term between Google and high-low impact. Environmental memberships variable added. N/A scores coded as 0
reg_GSen_VS_PopDen_MedianIncome_Cap_HL_State_Memberships_Inter <- lm(TimelineDays ~ View_Score + Google_Sentiment + PopDensity_mi + Household_MedianIncome + Capacity + H_L_W + State + Google_Sentiment*H_L_W, data=ventyx_df_NU_google_Sen)
