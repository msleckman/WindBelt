...
total_sentiment_NU <- total_sentiment_NU %>% 
  mutate(Extracted_ProjectName = sub("\\_.*$", "", document))
    # generate merge with extracted_projectName
    #verify: may be spaces in Ventyx project name

#Merge with original data (ventyx_df)
ventyx_df_NU_Sen <- merge(ventyx_df, total_sentiment_NU_df, by = "Extacted_ProjectName", all = TRUE)
