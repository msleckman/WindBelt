...
# By State:
 ...
 ByState <- Ventyx_dataset %>%
   select(ProjectName, ProjectDeveloper, State, lr_tif, Capacity, TimelineDays) %>% 
   group_by(State) %>% 
   mutate(LowImpact = sum(lr_tif == 1)) %>% 
   mutate(NonLowImpact = sum (lr_tif == 0)) %>% 
   mutate(TotalProjects = sum(lr_tif == 1 | lr_tif == 0)) %>%
   mutate(AvgCapacity = mean(Capacity)) %>% 
   mutate(AvgTimeline = mean(TimelineDays)) %>% 
   select(State,LowImpact, NonLowImpact, TotalProjects, AvgCapacity, AvgTimeline) %>% 
...
ByImpact <- Ventyx_dataset %>%
   select(ProjectName, ProjectDeveloper, State, lr_tif, Capacity, TimelineDays, Household_MedianIncome, View_Score) %>% 
   group_by(lr_tif) %>% 
   mutate(TotalProjects = sum(lr_tif == 1 | lr_tif == 0)) %>%
   mutate(AvgCapacity = mean(Capacity)) %>% 
  mutate(AvgTimeline = mean(TimelineDays)) %>%
  ...
  mutate(Avg_Viewscore = mean(View_Score)) %>% 
   select(TotalProjects, AvgCapacity, AvgTimeline, AvgMedianIncome, Avg_Viewscore) %>% 
   # Need to add column for sentiment
   unique()
