...
class Analysis: #create a class for running the analysis on each project
    ...
    def run(self): #define the main run function
        response = requests.get(self.url,headers=headers) #get the HTML text from the url
...
for project in range(len(unique_projects_list)): #for every unique project
    w = "wind project" #string that will be in quotes in search term
    random_time = random.randint(5,10) #randomize number between 5 and 10
    ...
    time.sleep(random_time) #sleep code by the random number of seconds

#print(a.term, ' Subjectivity: ', a.subjectivity, 'Sentiment: ', a.sentiment)
#print(columns['Master Project Name'])

with open('sentiment_analysis.csv', 'w') as csvfile: #open new file for writing
    filewriter = csv.writer(csvfile, delimiter=',',
                            quotechar='|', lineterminator='\n', quoting=csv.QUOTE_MINIMAL) 
    print("writing to file...")
    filewriter.writerow(['ProjectName','State','Sentiment','Subjectivity']) #write the header row of the file
    for i in range(len(unique_projects_list)): #loop through all unique projects
        filewriter.writerow([unique_projects_list[i],states_list[i],sentiment_list[i],subjectivity_list[i]]) #add all the details to the file
